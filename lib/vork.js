var url = require("url");
var fs = require("fs");
//var path = require("path");
function Vork(options) {
    var self = this;

    //Config Setup
    var configDefaults = {
        basepath: __dirname,
        DEBUG: false,
        modelsPath: 'models',
        viewsPath: 'views',
        elementsPath: 'elements',
        controlersPath: 'controlers',
        layoutsPath: 'layouts',
        webrootPath: 'webroot',
        helpersPath: 'helpers',
        DS: '/',
        cache: true,// set to for production
        EOL: '\r\n',
        dbConfig: 'db',
        port: Number(process.env.PORT || 80),
        globals: {}
    };

    this.config = {};

    if (typeof(options) === 'object') {
        for (var property in configDefaults) {
            if (!options[property]) this.config[property] = configDefaults[property];
            else this.config[property] = options[property];
        }
    }
    
    
    //Object Helpers
    this.tools = {
        checkFile: function(file) {
            try {
                fs.openSync(file + '.js', 'r');
                return true;
            }
            catch (e) {
                return false;
            }
        },
        sandbox: function(mvcData) {
            return {
                mvc: mvcData,
                //sandbox gets sent to all MVC objects so the oject can overide or gather info about current state also keeps MVC object from Killing the vork object
                config: self.tools.clone(self.config),
                //read-only config so if changed it wont effect anything except for what is running inside sandbox
                get: self.get
            };
        },
        clone: function(obj) {
            function Clone() {}
            return (function(obj) {
                Clone.prototype = obj;
                return new Clone();
            })(obj);
        }
    };
    
    var _dbFile = this.config.basepath + this.config.DS + this.config.dbConfig;
    if(self.tools.checkFile(_dbFile))
        this.db = require(_dbFile);
    else
        this.db = {};
    
    
    this.get = {     
        helper: function(helperName) {
            var jsFilename = self.config.basepath + self.config.DS + self.config.helpersPath + self.config.DS + helperName;
            if (self.tools.checkFile(jsFilename)) {
                var sandbox = self.tools.sandbox(arguments.callee.caller.arguments[0])
                var helper = require(jsFilename);
                if (typeof(helper) === 'function') 
                    sandbox = helper(sandbox);
                else sandbox =helper;
                if(!self.config.cache)
                    delete require.cache[jsFilename+'.js'];
                return sandbox;
            }
            else return null;
        },
        model: function(modelName) {
            var jsFilename = self.config.basepath + self.config.DS + self.config.modelsPath + self.config.DS + modelName;
            if (self.tools.checkFile(jsFilename)) {
                var sandbox = self.tools.sandbox(arguments.callee.caller.arguments[0])
                sandbox.mvc.db = self.db; //set so model can see db connection
                sandbox = require(jsFilename)(sandbox);
                if(!self.config.cache)
                    delete require.cache[jsFilename+'.js'];
                return sandbox;
            }
            else return null;
        },
        element: function(elementName) {
            var jsFilename = self.config.basepath + self.config.DS + self.config.elementsPath + self.config.DS + elementName;
            if (self.tools.checkFile(jsFilename)) {
                var sandbox = self.tools.sandbox(arguments.callee.caller.arguments[0])
                sandbox = require(jsFilename)(sandbox);
                if(!self.config.cache)
                    delete require.cache[jsFilename+'.js'];
            }
            else return null;
        }
    };
}

Vork.prototype.mvc = function mvc(req, res) {
    function compleateRequest(obj, res) {
        res.writeHead(obj.code, obj.headers);
        res.end(obj.req.method === "HEAD" ? "" : obj.content);
    }

    function checkRequest(obj, res) {
        if (obj && obj.code === 200) {
            compleateRequest(obj, res);
            return true;
        }
        return false;
    }
    function results404(req){
        var obj = {
                content: null,
                headers: null,
                code: 404,
                req: null
            };
            obj.req = req;
            
            obj.content = obj.req.url + ' not found - vorkNode';
            obj.headers = {
                "Content-Type": 'text/plain',
                "Content-Length": obj.content.length
            };
            compleateRequest(obj, res);
    }
    if(!checkRequest(this.loadAction(req), res))
            results404(req);
};

Vork.prototype.loadAction = function loadAction(req) {
    var DidIFail = false; //Never Fail Unless We Do Fail!
    var jsFilename; // holder for computed filename
    var defaults = { // mvcData Defaults
        controler: null,
        layout: 'default',
        action: 'index',
        view: null,
        params: [],
        contentType: 'text/html',
        db: null,
        req: null
    };
    defaults.req = req;
    
    var obj = {
        content: null,
        headers: null,
        code: 404,
        req: null
    };
    obj.req = req;
    
    var urlObj = url.parse(req.url);
    var filename = urlObj.pathname;
    if (filename == '/') filename = "/index";
    defaults.params = filename.substring(1, filename.length).split('/');

    defaults.params.reverse();
    if (defaults.params.length) {
        defaults.controler = defaults.params[defaults.params.length - 1];
        defaults.params.pop();
    }
    if (defaults.params.length) {
        defaults.action = defaults.params[defaults.params.length - 1];
        defaults.params.pop();
    }
    defaults.params.reverse();
    
    var controlerName = defaults.controler;
    jsFilename = this.config.basepath + this.config.DS + this.config.controlersPath + this.config.DS + defaults.controler;
    if (this.tools.checkFile(jsFilename)) {
        defaults.controler = require(jsFilename)[defaults.action];
        if(!this.config.cache)
            delete require.cache[jsFilename+'.js'];
        if (typeof(defaults.controler) === 'function') {
            defaults.controler = defaults.controler(this.tools.sandbox(defaults));
            DidIFail = false;
        }
        else DidIFail = true;
    }
    else {
        defaults.controler = null;
        DidIFail = true;
    }

    if (defaults.view === null) defaults.view = controlerName;
    jsFilename = this.config.basepath + this.config.DS + this.config.viewsPath + this.config.DS + defaults.view;
    if (this.tools.checkFile(jsFilename) && defaults.view !== false) {
        defaults.view = require(jsFilename)[defaults.view];
        if(!this.config.cache)
            delete require.cache[jsFilename+'.js'];
        if (typeof(defaults.view) === 'function') {
            defaults.view = defaults.view(this.tools.sandbox(defaults));
            obj.content = defaults.view;
            DidIFail = false;
        }
        else DidIFail = true;
    }
    else {
        DidIFail = true;
        defaults.view = null;
    }
    
    if (DidIFail) return obj;
    
    jsFilename = this.config.basepath + this.config.DS + this.config.layoutsPath + this.config.DS + defaults.layout;
    if (this.tools.checkFile(jsFilename)) {
        defaults.layout = require(jsFilename)[defaults.layout];
        if(!this.config.cache)
            delete require.cache[jsFilename+'.js'];
        if (typeof(defaults.layout) === 'function') {
            defaults.layout = defaults.layout(this.tools.sandbox(defaults));
            obj.content = defaults.layout;
        }
    }
    else {
        defaults.view = null;
    }
    
    jsFilename = null;

    if (obj.content) {
        obj.headers = {
            "Content-Type": defaults.contentType,
            "Content-Length": obj.content.length
        };
        if (!this.config.DEBUG) obj.headers["Cache-Control"] = "public";
        obj.code = 200;
        return obj;
    }
    return obj;
};


module.exports = function(opt) {
    return new Vork(opt);
};